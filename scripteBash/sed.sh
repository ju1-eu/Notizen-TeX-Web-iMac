...
for i in *.html; do
	...
	#sed -i -e '/"Grafiken/ s//"..\/Grafiken/g' "$i"
...
for i in *.html; do
	...
	sed -i -e 's/<embed/<img/g' "$i"
	sed -i -e '/.eps/ s//.'$bildformat'/g' "$i"
	sed -i -e '/.pdf/ s//.'$bildformat_2'/g' "$i"
	sed -i -e '/”/ s//\&laquo;/g' "$i"
	sed -i -e '/“/ s//\&raquo;/g' "$i"
done
...
for i in *.tex; do
	...
	sed -i -e '/\\begin{figure}/ s//\\begin{figure}[!hb]% hier: !hb/g' "$i"
